...
while IFS= read -r line; do
  ...
  then
    ...
    if [ `echo "${items[3]} % 50" | bc` -eq 49 ]
    then
      #while [ `ls -al /mnt/volume_1TB/atsai/script/merge/*.hap | wc -l` -lt `echo "${items[3]} - 999" | bc` ]
